namespace LIUZHU{
	const int MAXN;
	int from[MAXN + 10][MAXN * 2 + 10];
	int n,m;
	int edge[MAXN + 10][MAXN * 2 + 10];
	int sel[MAXN * 2 + 10],fa[MAXN * 2 + 10];
	int vis[MAXN * 2 + 10];
	const int INF;// INF >= sum( W_ij )
	int getfa(int x){
		if(x == fa[x]) return x;
	...
	void liuzhu(){ // 1-base: root is 1, answer = (sel[i], i) for i in [2..n]
		...
		for(int i = 2; i <= n; ++i){
			...
			for(int j = 1;j <= n; ++j) if(fa[j] != i){
				from[j][i] = i;
				if(edge[sel[i]][i] > edge[j][i]) sel[i] = j;
			}
		}
		int limit = n;
		while(1){
			...
			for(int i = 2; i <= prelimit; ++i) if(fa[i] == i && !vis[i]){
				...
				for(int j = 1; j <= n; ++j) if(getfa(j) == limit){
					edge[j][limit] = INF;
				}
				sel[limit] = 1;
				for(int j = 1; j <= n; ++j){
