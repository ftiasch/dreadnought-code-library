namespace Schreier_Sims_Algorithm{
	struct Permutation{
		vector<int> P;
		Permutation(){}
		Permutation(int n){
	...
	int n, m;
	vector<Bucket> buckets, bucketsInv;
	vector<Table> lookupTable;
	...
	bool inGroup(const Permutation &g){
	...
	void solve(const Bucket &gen,int _n){// m perm[0..n - 1]s
		n = _n, m = gen.size();
		{//clear all
			vector<Bucket> _buckets(n);
			swap(buckets, _buckets);
			vector<Bucket> _bucketsInv(n);
			swap(bucketsInv, _bucketsInv);
			vector<Table> _lookupTable(n);
			swap(lookupTable, _lookupTable);
		}
		for(int i = 0; i < n; ++i){
